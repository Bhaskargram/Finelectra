...
const axios = require('axios'); // --- NEW: Added axios dependency ---

// --- 2nd Gen Environment Variables ---
...
const ADMIN_PASSWORD = defineString('ADMIN_PASSWORD');
...
const connect = async () => {
    if (conn == null) {
        // Use .value() to access the defined environment variable
        conn = mongoose.connect(MONGO_URI.value(), {
...
// --- Database Schemas ---
const modelExists = (name) => mongoose.models[name];
...
const Setting = modelExists('Setting') ? mongoose.model('Setting') : mongoose.model('Setting', SettingSchema);
const Position = modelExists('Position') ? mongoose.model('Position') : mongoose.model('Position', PositionSchema);
const Election = modelExists('Election') ? mongoose.model('Election') : mongoose.model('Election', ElectionSchema);
const Candidate = modelExists('Candidate') ? mongoose.model('Candidate') : mongoose.model('Candidate', CandidateSchema);
const Voter = modelExists('Voter') ? mongoose.model('Voter') : mongoose.model('Voter', VoterSchema);
const QRCodeModel = modelExists('QRCode') ? mongoose.model('QRCode') : mongoose.model('QRCode', QRCodeSchema);
const Vote = modelExists('Vote') ? mongoose.model('Vote') : mongoose.model('Vote', VoteSchema);
const ArchivedResult = modelExists('ArchivedResult') ? mongoose.model('ArchivedResult') : mongoose.model('ArchivedResult', ArchivedResultSchema);
const ActiveSession = modelExists('ActiveSession') ? mongoose.model('ActiveSession') : mongoose.model('ActiveSession', ActiveSessionSchema);
const ScanLog = modelExists('ScanLog') ? mongoose.model('ScanLog') : mongoose.model('ScanLog', ScanLogSchema);
...
const auth = (req, res, next) => {
    ...
    try { 
        // Use .value() to access the defined environment variable
        req.user = jwt.verify(token, JWT_SECRET.value()); 
        next(); 
    } catch (e) { 
...
apiRouter.post('/auth/voter-login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(payload, JWT_SECRET.value(), { expiresIn: '12h' });
...
apiRouter.post('/auth/admin-login', (req, res) => {
    ...
    if (username === ADMIN_USERNAME.value() && password === ADMIN_PASSWORD.value()) {
        ...
        const token = jwt.sign(payload, JWT_SECRET.value(), { expiresIn: '8h' });
...
adminRouter.get('/voters', async (req, res) => { await connect(); res.json(await Voter.find().select('-password')); });
...
adminRouter.get('/candidates', async (req, res) => { await connect(); res.json(await Candidate.find()); });
adminRouter.post('/candidates', async (req, res) => { await connect(); res.status(201).json(await new Candidate(req.body).save()); });
...
adminRouter.put('/election/status', async (req, res) => { await connect(); res.json(await Election.findOneAndUpdate({}, req.body, { new: true, upsert: true })); });
...
adminRouter.get('/positions', async (req, res) => { await connect(); res.json(await Position.find()); });
adminRouter.post('/positions', async (req, res) => { await connect(); res.status(201).json(await new Position({ name: req.body.name }).save()); });
adminRouter.delete('/positions/:id', async (req, res) => { await connect(); res.json(await Position.findByIdAndDelete(req.params.id)); });
...
adminRouter.get('/generate-qrcodes', async (req, res) => {
    try {
        ...
        // This logic now safely adds new codes without checking election state,
        // allowing generation of more codes even during an 'Active' election.
        ...
        for (let i = 0; i < count; i++) {
            ...
            await new QRCodeModel({ token }).save();
...
adminRouter.get('/archived-results', async (req, res) => { await connect(); res.json(await ArchivedResult.find().sort({ year: -1 })); });

// --- MODIFIED: Complete overhaul of the PDF download endpoint with alignment fix ---
